...
import { getNoiseBackground } from "@/app/(homepage)/(material)/noise/noiseMaterial";
...
export class Environment {
    ...
    visible: boolean
    frameByFrameStatus: boolean
    oldFrameByFrameStatus: boolean
    name?: string

    // ensure this points to the instance
    onResize = () => this.reflow.call(this)

    constructor(rendererParameters: THREE.WebGLRendererParameters, bypassObserver?: boolean) {
        ...
        this.frameByFrameStatus = false
        this.oldFrameByFrameStatus = false
        this.reflow()
        ...
        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (this.name) {
                    console.log(`Visibility of ${this.name}: ${this.visible}`)
                }
                ...
                if (this.name) {
                    console.log(`Visibility of ${this.name} changed: ${entry.isIntersecting}`)
                }

                // visibility change
                const old = this.oldFrameByFrameStatus
                this.oldFrameByFrameStatus = this.frameByFrameStatus
                if (entry.isIntersecting && old) {
                    // resume rendering
                    this.startFrameByFrameRendering()
                } else if (this.frameByFrameStatus) {
                    // if rendering but not visible, stop rendering
                    this.stopFrameByFrameRendering()
                }
    ...
    removeObjects(container: THREE.Scene | THREE.Object3D) {
...
export class AnimatedEnvironment extends Environment {
    ...
    initialize() {
        ...
        this.oldFrameByFrameStatus = true
        this.render()
    }
    ...
    stopFrameByFrameRendering() {
        super.stopFrameByFrameRendering()
